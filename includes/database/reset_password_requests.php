<?php
  ...
  class ResetPasswordRequestMapping {
    ...
    public static function insert($sql_conn, $user_id, $reset_code) {
      ...
      check_db_error($sql_conn, $err_prefix, $stmt = $sql_conn->prepare('INSERT INTO reset_password_requests (user_id, reset_code, reset_code_sent, reset_code_used, created_at, updated_at) VALUES (?, ?, 0, 0, NOW(), NOW())'));
    ...
    public static function update_used($sql_conn, $id) {
      ...
      check_db_error($sql_conn, $err_prefix, $stmt = $sql_conn->prepare('UPDATE reset_password_requests SET reset_code_used=1, updated_at=NOW() WHERE id=?'));
      check_db_error($sql_conn, $err_prefix, $stmt->bind_param('i', $id));
      check_db_error($sql_conn, $err_prefix, $stmt->execute());
